...
void update() {
    ...
    if ( init_laser && init_robot ) {
        ...
        nb_pts = 0;
        if ( !current_robot_moving ) {// current state -- stand
            //if the robot is not moving then we can perform moving person detection          
            ROS_INFO("robot is not moving");
            //DO NOT FORGET to store the background but when ??? - yes
            /*
            if (!previous_robot_moving) // previous state --> stand
            {
               detect_motion(); // detect - motion to store  --> Receive dynamic_point
            }
            store_background();// store the background for next comparison ---> find dynamic point between two state (previous and current)
            */
            if (!current_robot_moving && previous_robot_moving)
            {
                store_background();
            }
        ...
        else
        {
            ...
            {
            	reset_motion();// refesh the dynamic point --> turn all into 0
            }
    ...
    ROS_INFO("detecting motion");
     ...
     for (int loop=0; loop<nb_beams; loop++ )
      {//loop over all the hits
        //the detect of motion ONLY takes place when the robot is not moving, ie when current_robot_moving is false --> update void
        //when current_robot_moving is true, dynamic[loop] is false for all the beams --> update void 

        //if the difference between ( the background and the current range ) is higher than "detection_threshold"
        if (abs(background[loop]-r[loop]) > detection_threshold) 
        {       
...
*/
    for( int loop=1; loop<nb_beams; loop++ )//loop over all the hits
    {
        ...
        {//the current hit doesnt belong to the same hit
              ...
              cluster[loop-1] = nb_cluster;// store for each hit , the cluster it belongs to
      	      //nb_dynamic++; // to count the number of hits of the current cluster that are dynamic
	      ...
	      //1/ we end the current cluster, so we update:
             ...
             cluster_size[nb_cluster]=distancePoints(current_scan[start],current_scan[end]);
              ...
              cluster_middle[nb_cluster].x = (current_scan[start].x + current_scan[end].x)/2;
              cluster_middle[nb_cluster].y = (current_scan[start].y + current_scan[end].y)/2;
              
             // - cluster_dynamic to store the percentage of hits of the current cluster that are dynamic
             if ( dynamic[end] ) {nb_dynamic++ ;}
...
// if more than "dynamic_threshold"% of its hits are dynamic the leg is considered to be dynamic
    ...
    {
        /*if the size of the current cluster is higher than "leg_size_min" and lower than "leg_size_max" then the current cluster is a leg*/
        //add
           ...
           // - the leg_dynamic to know if the leg is dynamic or not.                     
            ...
            if ( leg_dynamic[nb_legs_detected]  )
            {
                ...
                for(int loop2=0; loop2<nb_beams; loop2++)
                    ...
                    //person_dynamic[nb_legs_detected]=true;
            }
            else
            {
...
// a moving person (ie, person_dynamic array) has 2 legs that are dynamic
    ...
    for (int loop_leg1=0; loop_leg1<nb_legs_detected; loop_leg1++)//loop over all the legs
    {
            ...
            if ( distancePoints(leg_detected[loop_leg1],leg_detected[loop_leg2])<legs_distance_max)
	    {
            ...
            person_detected[nb_persons_detected].x=(leg_detected[loop_leg2].x+leg_detected[loop_leg1].x)/2;
            person_detected[nb_persons_detected].y=(leg_detected[loop_leg2].y+leg_detected[loop_leg2].y)/2;
...
void detect_a_moving_person() {
    ...
    for (int loop=0; loop<nb_persons_detected; loop++)
    {
        if ( person_dynamic[loop])
        {
            ...
            moving_person_detected = person_detected[loop]; //publish the location of moving person
