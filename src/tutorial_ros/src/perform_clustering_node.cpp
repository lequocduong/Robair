...
//then we end the current cluster with the previous hit and start a new cluster with the current hit
    ...
    for( int loop=1; loop<nb_beams; loop++ )//loop over all the hits
        {
           if (distancePoints(current_scan[loop],current_scan[start])>cluster_threshold)
           //if (sqrt(pow(current_scan[loop].x - current_scan[start].x,2.0)  + pow(current_scan[loop].y - current_scan[start].y,2.0) ) > cluster_threshold )
        {//the previous hit is the end of the current cluster
            geometry_msgs::Point middle;

            //we end the current cluster - last hit of current cluster
            end = loop-1;
	    ...
	    // compute the middle of the cluster
            middle.x = (current_scan[start].x + current_scan[end].x)/2;
            middle.y = (current_scan[start].y + current_scan[end].y)/2;// compute the middle of the cluster
