...
public:
    ...
    void update() {
        ...
        if (init_odom && init_laser && init_position) {
            ...
            if ((distance_traveled > distance_to_travel) || (fabs(angle_traveled * 180 / M_PI) > angle_to_travel)) {
                predict_position(); // update the predicted position and orientation
                estimate_position();
            }
        }
    ...
    void initialize_localization() {
        ...
        ROS_INFO("initial_position(%f, %f, %f): score = %i",
                 initial_position.x,
                 initial_position.y,
        ...
        // My code ----------------------------------------------------------------------
        // - init: search the position with the highest sensor_model in a square of ...
        // 2x2 meters around the initial_position and with all possible orientations.
        // - the increment on x and y is of 5cms and on theta is of 5 degrees
        // loop_x, loop_y [m], o [degree]

        min_x = initial_position.x - 1.0;
        max_x = initial_position.x + 1.0;
        min_y = initial_position.y - 1.0;
        max_y = initial_position.y + 1.0;
        min_orientation = initial_orientation - 180 * M_PI / 180; // [rad]
        max_orientation = initial_orientation + 180 * M_PI / 180; // [rad]
            ...
            for (float loop_y = min_y; loop_y < max_y; loop_y += 0.05)
                for (float o = min_orientation; o < max_orientation; o += 5 * M_PI / 180) {
                    if (cell_value(loop_x, loop_y) == 0) { // robair can only be at a free cell
                        ...
                        getchar();
                        //we store the maximum score over all the possible positions in estimated_position
                        if (score_current > score_largest) {
    ...
    void estimate_position() {
        ...
        min_x = initial_position.x - 0.5;
        max_x = initial_position.x + 0.5;
        min_y = initial_position.y - 0.5;
        max_y = initial_position.y + 0.5;
        min_orientation = initial_orientation - 30 * M_PI / 180; // [rad]
        max_orientation = initial_orientation + 30 * M_PI / 180; // [rad]
    ...
    int sensor_model(float x, float y, float o) {
        ...
        for (int loop = 0; loop < nb_beams; loop++, beam_angle += angle_inc) {
            ...
            // My code -----------------------------------------------------------------
            // hit: position of the beam in global frame
            // current_scan: position of the beam in local frame
